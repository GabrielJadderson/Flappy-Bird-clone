 ...
 * 3. Read the PCM data using one of the read functions, and feed it to your
 * player.
 * <p>
...
public class OggInputStream extends FilterInputStream {
		...
		while (!eos && !fetchedPacket) {
			...
			if (result1 == 0) {
				...
				if (result2 == 0) {
					...
					// throw new
					// Exception("syncState.pageout(page) result == -1");
				...
				// throw new
				// Exception("streamState.packetout(packet) result == -1");
				System.out
				.println("streamState.packetout(packet) result == -1");
				return -1;
			} else {
				fetchedPacket = true;
			}
	...
	public static void main(String args[]) {
		try {
			InputStream in = new Object().getClass().getResourceAsStream("sounds/Default.0005.ogg");
